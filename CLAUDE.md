...
## Changelog
- **2026-05-19**: **AI-Authored Scenario** — New `RimMind: AI-Authored Story` scenario in the picker. Player types a one-line pitch, AI returns a full plan: world lore, ideology (name + description, applied to the in-game `Ideo` via `Traverse` on game start), recent events, tech level, themes, party origin (how-they-met / shared-goal / internal-tension), inciting incident, active forces, planted seeds, plus per-pawn name/age/appearance/childhood/adulthood/defining-moment, traits, skills, passions, and 8–15 starting items + biome/season hints. Schema is editable in a modal dialog (every field is a `TextArea`) before commit. Persisted twice: on the `ScenPart_RimMindCampaign` and mirrored into `NarrativeEngine.state.Campaign`. `CampaignFrame.BuildPromptContext` now includes the rich lore so every storyteller plan call sees it. Reasoning models like Qwen3.5 are handled via `chat_template_kwargs.enable_thinking=false`. Pre-game `MainThreadDispatcher.Drain` was added because `GameComponentUpdate` doesn't fire when `Current.Game` is null. Harmony patches `Page.DoNext` (not `Page_SelectScenario.DoNext` which doesn't exist; not `BeginScenarioConfiguration` which sets `this.next`). Replaces the post-spawn pop-up. See [docs/ai-scenario.md](docs/ai-scenario.md) for the full feature spec. Related: [#175 — AI Storyteller RFC](https://github.com/Bwarhness/RimMind/issues/175).
