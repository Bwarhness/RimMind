        ...
        public static string GetTemperatureRisks()
        {
            ...
            foreach (var colonist in colonists)
            {
                ...
                if (localTemp < minComfort - 5f)
                {
                    ...
                    var warmRoom = FindWarmRoom(map);
                    if (warmRoom != null)
                    {
                        obj["suggested_location"] = warmRoom.Value["label"]?.Value;
                        obj["suggested_position"] = warmRoom.Value["position"]?.Value;
                    }
                ...
                else if (localTemp > maxComfort + 5f)
                {
                    ...
                    var coolRoom = FindCoolRoom(map);
                    if (coolRoom != null)
                    {
                        obj["suggested_location"] = coolRoom.Value["label"]?.Value;
                        obj["suggested_position"] = coolRoom.Value["position"]?.Value;
                    }
